...
const ShopProducts = ({ products = [] }) => {
  ...
  const changeQty = (id, delta, max = 99) => {
    setQuantities((prev) => {
      ...
      const next = Math.min(Math.max(1, curr + delta), max);
      return { ...prev, [id]: next };
    });
  };
  ...
  const renderStars = (rating) => {
    return [...Array(5)].map((_, i) => (
      <span key={i} className={i < Math.floor(rating) ? "text-yellow-400" : "text-gray-100"}>
        <FaStar />
      </span>
    ));
  };
    ...
    <div className="mb-8 sm:mb-12 border-1 border-gray-200 rounded-lg">
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-0">
        {products.map((product) => {
          ...
          const unit = Number(product.discount ? product.finalPrice : product.price);
          const subTotal = (unit * qty).toFixed(2);
          return(
          <div
            key={product.id}
            className="bg-white border border-gray-200 p-5 shadow-sm hover:shadow-xl transition-shadow duration-300 flex flex-col h-full"
          >
            ...
            <div className="flex flex-col justify-between flex-grow relative">
              <h3 className={`text-xs sm:text-sm md:text-md font-medium tracking-normal  text-gray-800 ${product.title.length > 28 ? "mb-2 sm:mb-2":" mb-6 sm:mb-7"}`}>
                {product.title.length > 40
                  ? product.title.substring(0, 40) + "..."
                  : product.title}
              </h3>
              <p className={`text-xs font-semibold uppercase text-green-600 mb-4 tracking-tight`}>
